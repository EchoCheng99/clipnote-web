...
1. 用中文简要解析这段文字的语法结构（句子成分、从句、时态、固定结构等），控制在150字以内。
2. 从这段选中文字里，挑出1-4个值得学习者积累的高频表达/短语/句型（不要挑太基础的单词），每个给出：英文原形、中文释义、这个表达在原文中所在的那一整句话（必须是原文逐字摘录，不要自己改写或新造句子，标点和大小写都照抄原文）、这句原文的中文翻译、1个近义表达（没有则留空字符串）。

只返回严格JSON，不要markdown代码块标记，不要任何多余文字，格式为：
{"grammar_explanation": "中文语法解析", "collocations": [{"en": "表达", "zh": "中文释义", "example": "原文中包含该表达的完整原句（逐字摘录）", "translation": "该原句的中文翻译", "synonym": "近义表达或空字符串"}]}`;
